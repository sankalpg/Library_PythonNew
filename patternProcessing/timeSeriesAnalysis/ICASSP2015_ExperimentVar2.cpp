...
int main( int argc , char *argv[])
{
    ...
    {
        ind1 = ii*nInterFact;
        pattLenFinal = TSData1->subSeqPtr[ind1].len;//max(TSData1->subSeqPtr[ind1].len, TSData1->subSeqPtr[ind2].len);
        ...
        for(jj=0; jj<nCands; jj++)
        {
            ...
            for(ss=0; ss< nInterFact; ss++)
            {
                for(kk=0; kk< nInterFact; kk++)
                {
                    ...
                    copyLinStrechedBuffer(buff2, TSData1->subSeqPtr[ind2+kk].pData, pattLenFinal, pattLenFinal);
